...
<head lang="ko">
  ...
  <%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
	 ...
	 $(document).ready(function(){ 
		 $("li[value='${pageMaker.examDto.page}']").attr("class","on");
...
<div class="manage schedule">
  <div class="contentBoxAd">
    ...
    <h2>채점표리스트</h2>
    <form action="/resultListIntro" method="get">
	...
	<form action="/excelDownload" method="POST" name="excelForm" id="excelForm">
		<input type="hidden" name="menuId" id="menuId" value="resultListIntro">
		<input type="hidden" name="yearConditionForExcel" id="yearConditionForExcel" value="${yearCondition}">
		<input type="hidden" name="examNameConditionForExcel" id="examNameConditionForExcel" value="${examNameCondition}">
		<input type="hidden" name="degreeConditionForExcel" id="degreeConditionForExcel" value="${degreeCondition}">
		<input type="hidden" name="onePageDataCountConditionForExcel" id="onePageDataCountConditionForExcel" value="${pageCondition}">
		<input type="hidden" name="pageForExcel" id="pageForExcel" value="${examDto.page}">
		
        <input type="hidden" name="fileName" id="fileName" value="">   
        <input type="hidden" name="columns" id="columns" value="">   
        <input type="hidden" name="data" id="data" value="">
        <input type="hidden" name="docName" id="docName" value="채점표리스트">   
     </form>
    <ul class="btn_wrap">
    ...
    <table class="mb100" id="columnList">
      ...
      <c:forEach var="examList" items="${examList}" varStatus="status"> 
      	<tr class="center">
        <td><input type="checkbox" name="examCheck" value="${examList.examId}"></td>
        <td>${status.count}</td>
        <td>${examList.examYear}</td>
	...
	<ul class="btn-group pagination">
	 <c:if test="${pageMaker.prev}">
	     <li>
	        <a href='<c:url value="/resultListIntro?page=${pageMaker.startPage-1}&yearCondition=${yearCondition}&examNameCondition=${examNameCondition}&degreeCondition=${degreeCondition}&onePageDataCountCondition=${pageCondition}" />'><i class="fa fa-chevron-left">이전</i></a>
	    </li>
	</c:if>
	 <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="pageNum">
	   <li value="${pageNum}"> 
	         <a href='<c:url value="/resultListIntro?page=${pageNum}&yearCondition=${yearCondition}&examNameCondition=${examNameCondition}&degreeCondition=${degreeCondition}&onePageDataCountCondition=${pageCondition}" />'><i class="fa">${pageNum}</i></a>
	   </li>
	</c:forEach>
	<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
	   <li>
	        <a href='<c:url value="/resultListIntro?page=${pageMaker.endPage+1}&yearCondition=${yearCondition}&examNameCondition=${examNameCondition}&degreeCondition=${degreeCondition}&onePageDataCountCondition=${pageCondition}"/>'><i class="fa fa-chevron-right">다음</i></a>
	     </li>
	</c:if>
	</ul>   
